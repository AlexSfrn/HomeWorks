...
const UniqArray = function(array) {
    var NewArray = [];
    for (let num = 0; num < array.length; num++) {
        if (NewArray.indexOf(JSON.stringify(array[num])) === -1) {
            NewArray.push(JSON.stringify(array[num]));
        }
    }
    for (let num1 = 0; num1 < NewArray.arraylength; num1++) {
        JSON.parse(NewArray(num1))
    }
    return NewArray;
}

console.log(UniqArray(LIST))
console.log(UniqArray(LIST).length)
...
const fileJSON = function(file) {
    const fs = require('fs');
    let data = fs.readFileSync('./task2.json');
    let JSONfile = JSON.parse(data);
    return JSONfile
}

console.log(fileJSON('task2.json'))
console.log(fileJSON('task2.json').length)